...
- (void)viewDidLoad
{
    ...
    _quadRenderer = [[QuadRenderer alloc] initWithDefaultFBOName:_defaultFBOName];
...
- (void)viewDidLayout
{
    ...
    [_quadRenderer resize:viewSizePixels];
...
{
    _quadRenderer.mouseCoords = point;
}
...
- (void)mouseDown:(NSEvent *)event
{
    ...
    _quadRenderer.mouseCoords = mousePoint;
}
               ...
               error:(NSError **)error
{
    ...
    // The following call should return 0x8814 which is GL_RGBA32F
    ...
    printf("Texture Internal Format:0x%0X\n", format);
...
- (void)keyDown:(NSEvent *)event
{
    if( [[event characters] length] ) {
        ...
        if (nKey == 83 || nKey == 115) {
            GLuint textureID = _quadRenderer.colorCubemapTexture;
            if (textureID != 0) {
